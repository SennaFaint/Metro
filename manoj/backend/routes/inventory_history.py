...
def create_inventory_snapshot():
    ...
    snapshot_date = data.get("snapshot_date")  # Should be YYYY-MM-DD format
    ...
    # Parse snapshot date - normalize to midnight UTC
    if snapshot_date:
        try:
            # If snapshot_date is just YYYY-MM-DD, parse it and set to midnight UTC
            if len(snapshot_date) == 10:  # YYYY-MM-DD format
                ...
                # Normalize to midnight UTC for consistent matching
    ...
    # Get today's date (normalized to midnight UTC) for comparison
